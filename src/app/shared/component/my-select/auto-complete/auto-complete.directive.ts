import {
    ChangeDetectorRef, ComponentFactoryResolver, ComponentRef, Directive,
...
export class NguiAutoCompleteDirective implements OnInit {
  ...
  @Input("byAttrib") byAttrib: string;
  ...
  @Input("show-dropdown-on-init") showDropdownOnInit: boolean ;
  ...
  ngAfterViewInit() {
    ...
    if(this.showDropdownOnInit==true){
       setTimeout(() => {
        this.inputEl.focus();
  ...
  showAutoCompleteDropdown = (event?: any): void => {
    ...
    component.displayPropertyName=this.displayPropertyName
    component.showDropdownOnInit=this.showDropdownOnInit;
    component.byAttrib = this.byAttrib;
  ...
  private keydownEventHandler = (evt: any) => {
    ...
    if (this.componentRef) {
      const component = <NguiAutoCompleteComponent>this.componentRef.instance;
